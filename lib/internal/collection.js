...
function createSet([iterateArray, iterateObject]) {
  ...
  return function set(collection) {
    if (Array.isArray(collection)) {
      ...
      this._iterate = typeof this._iterator === 'function' ? iterateArray : iterateArrayShorthand;
    } else if (collection && typeof collection === 'object') {
      const keys = Object.keys(collection);
      this._coll = collection;
      this._size = keys.length;
      this._keys = keys;
      this._iterate = typeof this._iterator === 'function' ? iterateObject : iterateObjectShorthand;
    } else {
      this._size = 0;
    }
