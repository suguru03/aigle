...
function setShorthand(collection) {
  if (Array.isArray(collection)) {
    this._coll = collection;
    this._size = collection.length;
    this._iterate = typeof this._iterator === 'function' ? iterateArrayParallel : iterateArrayShorthand;
  } else if (collection && typeof collection === 'object') {
    const keys = Object.keys(collection);
    this._coll = collection;
    this._size = keys.length;
    this._keys = keys;
    this._iterate = typeof this._iterator === 'function' ? iterateObjectParallel : iterateObjectShorthand;
  } else {
    this._size = 0;
  }
  this._rest = this._size;
...
function iterateArrayShorthand() {
  const { _iterator, _coll } = this;
  let l = this._rest;
  while (l--) {
    const obj = _coll[l];
    if (obj) {
      this._callResolve(obj[_iterator], l);
    } else {
      this._callResolve(undefined, l);
    }
  }
}

function iterateObjectShorthand() {
  const { _iterator, _coll, _keys } = this;
  let l = this._rest;
  while (l--) {
    const obj = _coll[_keys[l]];
    if (obj) {
      this._callResolve(obj[_iterator], l);
    } else {
      this._callResolve(undefined, l);
