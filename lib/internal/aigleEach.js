...
class AigleEachArray extends AigleProxy {
  ...
  constructor(array, iterator) {
    ...
    this._rest = size;
    this._result = array;
    this._iterate(iterator, size);
  }

  _iterate(iterator, size) {
    let i = -1;
    const { _result } = this;
    while (++i < size) {
      const p = call2(iterator, _result[i], i);
      if (p === errorObj) {
...
class AigleEachObject extends AigleProxy {
  ...
  constructor(object, iterator) {
    ...
    this._rest = size;
    this._result = {};
    this._iterate(iterator, size, object, keys);
  }

  _iterate(iterator, size, object, keys) {
    let i = -1;
    while (++i < size) {
      const key = keys[i];
