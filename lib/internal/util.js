...
const [
  ...
  createPromiseArrayEach,
  createPromiseObjectEach,
  createPromiseSetEach,
  createPromiseMapEach
].map(createFunc => [callProxyReciever, callProxyRecieverWithFunc].map(createFunc));
...
function createPromiseArrayEach(handler) {
  return function promiseArrayEach(receiver, times, coll) {
    let i = -1;
    while (++i < times && handler(coll[i], receiver, i)) {}
  };
}

function createPromiseObjectEach(handler) {
  return function promiseObjectEach(receiver, times, coll, keys, result) {
    let i = -1;
    let key;
    while (++i < times) {
      key = keys[i];
      result[key] = undefined;
      if (handler(coll[key], receiver, key) === false) {
        return;
      }
    }
  };
...
function createPromiseSetEach(handler) {
  return function promiseSetEach(receiver, times, coll) {
    const iter = coll[iteratorSymbol]();
    let i = -1;
    let item;
    while (
      ++i < times &&
      (item = iter.next()).done === false &&
      handler(item.value, receiver, i)
    ) {}
  };
}

function createPromiseMapEach(handler) {
  return function promiseMapEach(receiver, times, coll, result) {
    const iter = coll[iteratorSymbol]();
    let i = -1;
    let item;
    while (++i < times && (item = iter.next()).done === false) {
      const [key, promise] = item.value;
      result.set(key, promise);
      if (handler(promise, receiver, key) === false) {
        return;
      }
    }
  };
