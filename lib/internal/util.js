...
] = [createPromiseArrayEach, createPromiseObjectEach, createPromiseSetEach, createPromiseMapEach].map(createFunc =>
  [callProxyReciever, callProxyRecieverWithFunc].map(createFunc)
);
...
function createPromiseArrayEach(handler) {
  return function promiseArrayEach(receiver) {
    ...
    let i = -1;
    while (++i < _rest && handler(_coll[i], receiver, i)) {}
  };
}
...
function createPromiseObjectEach(handler) {
  return function promiseObjectEach(receiver) {
    ...
    while (++i < _rest) {
      ...
      if (handler(_coll[key], receiver, key) === false) {
        return;
      }
    }
  };
...
function createPromiseSetEach(handler) {
  return function promiseSetEach(receiver) {
    ...
    let i = -1;
    let item;
    while ((item = iter.next()).done === false && handler(item.value, receiver, ++i)) {}
  };
}
