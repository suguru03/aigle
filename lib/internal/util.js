...
function createObjectIterator(handler) {
  return (receiver, coll, index, keys, result) => {
...
function createObjectEach(iterator) {
  return (receiver, times, coll, keys, result) => {
    let i = -1;
    while (++i < times && iterator(receiver, coll, i, keys, result)) {}
  };
}
...
function createMapIterator(handler) {
  return (receiver, iter, index, keys, result) => {
    const item = iter.next();
    if (item.done) {
      return false;
    }
...
function createMapEach(iterator) {
  return (receiver, times, coll, result) => {
    const iter = coll[iteratorSymbol]();
    let i = -1;
    while (++i < times && iterator(receiver, iter, i, undefined, result)) {}
  };
}
