  ...
  _resume() {
    ...
    case 1:
      while (l-- !== 0) {
        ...
        if (this._async) {
          task._resolve(this._value);
        } else {
          process.nextTick(() => task._resolve(this._value));
        }
      }
      break;
    case 2:
      ...
      while (l-- !== 0) {
        const task = this._queue.shift();
        if (this._async) {
          task._reject(this._value);
        } else {
          process.nextTick(() => task._reject(this._value));
        }
      }
      break;
    }
...
function resolve(value) {
  const p = new Promise(noop);
  p._callResolve(value);
  return p;
}

function reject(reason) {
  const p = new Promise(noop);
  p._callReject(reason);
  return p;
}
