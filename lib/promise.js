...
class Promise {
  ...
  spread(handler) {
    const promise = new Promise(INTERNAL);
    handler = createSpreadHandler(this, handler);
    if (this._resolved === 0) {
      this._addPromise(promise, handler);
    } else {
  ...
  _resolve(value) {
    ...
    } else if (_receiver instanceof Proxy) {
  ...
  _reject(reason) {
    ...
    } else if (_receiver instanceof PromiseArray) {
      _receiver._reject(reason);
    } else {
  ...
  _addPromise(receiver, onFullfilled, onRejected) {
    if (this._receiver === undefined) {
  ...
  _addReceiver(receiver, key) {
    ...
    this._key = key;
    this._receiver = receiver;
    this._addPromise(_receiver, _onFullFilled, _onRejected);
  }
}

class Proxy extends Promise {
  constructor() {
    super(INTERNAL);
  }
}

const PromiseArray = require('./internal/promiseArray')(Promise, Proxy);

Promise.resolve = resolve;
Promise.reject = reject;

Promise.join = require('./join')(Promise, PromiseArray);
...
function createSpreadHandler(promise, handler) {
  return () => {
    const { _resolved, _value } = promise;
    if (typeof handler !== 'function') {
      const p = new Promise(INTERNAL);
