...
function callResolve(promise, value) {
  while (promise) {
    ...
    const { key, finallyHandler, onFullfilled } = _receiver;
    ...
    const p = key === undefined ? tryCatch(onFullfilled, value) : tryCatchWithKey(onFullfilled, value, key);
    if (_receivers) {
      ...
      while (_receivers.head) {
        ...
        if (p instanceof Promise) {
          switch (p._resolved) {
          case 0:
            p._receiver = new Receiver(promise, promise.onFullfilled, promise.onRejected, promise.errorTypes, promise.finallyHandler);
            continue;
          case 1:
