...
class Promise {
  ...
  _resume() {
    ...
    case 1:
      if (this._async) {
        while (l-- !== 0) {
          this._queue.shift()._resolve(this._value);
        }
      } else {
        while (l-- !== 0) {
          process.nextTick(() => this._queue.shift()._resolve(this._value));
        }
      }
      break;
    case 2:
      if (l === 0) {
        return process.emit('unhandledRejection', this._value);
      }
      if (this._async) {
        while (l-- !== 0) {
          this._queue.shift()._reject(this._value);
        }
      } else {
        while (l-- !== 0) {
          process.nextTick(() => this._queue.shift()._reject(this._value));
        }
      }
      break;
    }
