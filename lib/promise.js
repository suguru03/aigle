...
class Promise {
  ...
  _resolve(value) {
    ...
    this._receiver = undefined;
    if (_receiver instanceof PromiseArray) {
      _receiver._resolve(value, _key);
    } else {
...
const PromiseArray = require('./internal/promiseArray')(Promise);
...
function callResolve(receiver, onFullfilled, value, key) {
  if (typeof onFullfilled !== 'function') {
    receiver._resolve(value);
    return;
  }
  const promise = key ? tryCatchWithKey(onFullfilled, value, key) : tryCatch(onFullfilled, value);
  if (promise === errorObj) {
