  ...
  while (promise) {
    ...
    if (_receivers) {
      ...
      while (_receivers.head) {
        ...
        const { promise, onFullfilled } = _receiver;
...
class Promise {
  ...
  then(onFullfilled, onRejected) {
    ...
    if (this._receiver) {
      this._receivers = this._receivers || new Queue();
      this._receivers.push(new Receiver(promise, null, onRejected));
      return promise;
    }
    this._receiver = promise;
    if (hasOnRejected) {
    ...
    if (this._receiver) {
      this._receivers = this._receivers || new Queue();
      this._receivers.push(new Receiver(promise, null, null, handler));
      return promise;
    }
    this._receiver = promise;
