...
/*
 * https://github.com/caolan/async/blob/v2.1.5/lib/autoInject.js#L7-L10
 */
...
function parseParams(handler) {
  handler = handler.toString()
    ...
    .match(FN_ARGS);
  handler = (handler[2] || handler[3] || '').replace(' ', '');
  if (!handler) {
    return [];
  }
  const result = handler.split(FN_ARG_SPLIT);
  let l = result.length;
  while (l--) {
    result[l] = result[l].replace(FN_ARG, '').trim();
  }
  return result;
}
