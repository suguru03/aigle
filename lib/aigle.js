...
class Aigle extends AigleCore {
  ...
  spread(handler) {
    const receiver = new Spread(handler);
    this._key = INTERNAL;
    this._resolved === 0 ? this._addReceiver(receiver) : push(this, receiver);
  ...
  delay(ms) {
    const receiver = new Delay(ms);
    this._resolved === 0 ? this._addReceiver(receiver) : push(this, receiver);
    return receiver._promise;
  }

  timeout(ms, message) {
    const receiver = new Timeout(ms, message);
    this._resolved === 0 ? this._addReceiver(receiver) : push(this, receiver);
    return receiver._promise;
  }
