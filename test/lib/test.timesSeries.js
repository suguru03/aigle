...
parallel('timesSeries', () => {
  it('should execute', () => {
    ...
    return Aigle.timesSeries(count, iterator).then((res) => {
  ...
  it('should return an empty array if times is not number', () => {
    const iterator = (n) => n * 2;
    return Aigle.timesSeries('test', iterator).then((res) => {
      assert.strictEqual(Object.prototype.toString.call(res), '[object Array]');
  ...
  it('should return an array even if iterator is undefined', () => {
    return Aigle.timesSeries(5).then((res) => {
      assert.strictEqual(Object.prototype.toString.call(res), '[object Array]');
  ...
  it('should catch a TypeError', () => {
    ...
    return Aigle.timesSeries(count, iterator).then(assert.fail).catch(TypeError, assert.ok);
  });
});
